...
* subType.coding.code ^short = "申報類別，1:送核 | 2:送核補件 | 3:申復 | 4:爭議審議 | 5:申復補件 。當申報類別為2(送核補件)、3(申復)、4(爭議審議)、5(申復補件)時，院所才需於Claim.identifier:old_acpt_no填寫原送核案件之受理編號。"
* enterer 1..1
...
* item.quantity from ApplQtyUnit
* item.quantity ^binding.description = "事前審查申請數量單位及包裝類型。"
...
* item.programCode.coding ^short = "給付適應症條件，當事前審查品項代碼為KC009612B5、KC010892B5、KC011112DE、KC011162B5、KC011362B5、KC01013229、KC01013230、KC01025219、KC01050238、KC01085229、KC00958229、JC00154261、KC01088229、KC01233224時，才可填寫。"
* item.programCode.text ^short = "給付適應症條件，事前審查品項代碼不為KC009612B5、KC010892B5、KC011112DE、KC011162B5、KC011362B5、KC01013229、KC01013230、KC01025219、KC01050238、KC01085229、KC00958229、JC00154261、KC01088229、KC01233224時，請自行輸入文字。"
...
* . obeys sequence-1 and applType and supportingInfo and supportingInfo-tests and supportingInfo-c90-c91-c92
...
Description: "當Claim.priority(案件類別)為1(一般事前審查申請)、3(自主審查)時，至少還需提供檢查報告、影像報告、基因資訊中任一樣資訊。如果Claim.diagnosis.diagnosisCodeableConcept(國際疾病分類代碼)為C61，且Claim.item.modifier:continuation(續用註記)為2，則可排除此條件。或Claim.diagnosis.diagnosisCodeableConcept(國際疾病分類代碼)為C90、C91或C92，則可排除此條件。"
Expression:  "(priority.coding.code.matches('1|3')) implies (supportingInfo.category.exists(coding.code = 'examinationReport') or supportingInfo.category.exists(coding.code = 'imagingReport') or supportingInfo.category.exists(coding.code = 'geneInfo') or (diagnosis.diagnosis.ofType(CodeableConcept).coding.where(code.matches('^(C61)')).exists() and item.modifier.where(coding.system = 'https://nhicore.nhi.gov.tw/pas/CodeSystem/nhi-continuation-status').coding.exists(code = '2')) or (diagnosis.diagnosis.ofType(CodeableConcept).coding.where(code.matches('^(C90|C91|C92)')).exists()))"
Severity:    #error
...
Invariant:   applType
Description: "當Claim.subType(申報類別)為2(送核補件)、3(申復)、4(爭議審議)或5(申復補件)時，院所才需於Claim.identifier:old_acpt_no填寫原送核案件之受理編號。目前IG僅針對申報類別為3(申復)進行檢核。"
Expression:  "(subType.coding.exists(code='3')) implies (identifier.where(use = 'secondary').exists())"
Severity:    #error
