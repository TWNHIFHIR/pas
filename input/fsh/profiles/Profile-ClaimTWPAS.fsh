...
* . obeys sequence-1 and applType and supportingInfo and supportingInfo-tests and supportingInfo-c90-c92
...
Invariant:   supportingInfo-c90-c92
Description: "當Claim.diagnosis.diagnosisCodeableConcept(國際疾病分類代碼)為C90或C92，需提供檢驗(查)、影像報告、基因資訊、檢驗檢查中任一項資訊。"
Expression:  "(diagnosis.diagnosis.ofType(CodeableConcept).coding.where(code.matches('^(C90|C92)')).exists()) implies (supportingInfo.category.exists(coding.code = 'tests') or supportingInfo.category.exists(coding.code = 'imagingReport') or supportingInfo.category.exists(coding.code = 'geneInfo') or supportingInfo.category.exists(coding.code = 'examinationReport'))"
Severity:    #error
